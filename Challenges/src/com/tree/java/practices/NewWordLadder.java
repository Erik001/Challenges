...
public class NewWordLadder {
	...
	public static int wordLadder(String beginWord, String endWord, String[] wordList) {
		...
		if(!hasPassedStartingPoint && !Arrays.asList(wordList).contains(beginWord)){
			++shortestTrans;
			hasPassedStartingPoint = true;
		}
		...
		if (mapOfDiffs.get(1) != null && mapOfDiffs.get(1).size() > 0 && mapOfDiffs.get(1).contains(endWord)) {
			...
			switch(shortestTrans){
			case 1:
				System.out.printf("Found at %dst Transformation!! %n%n",shortestTrans);
				break;
			case 2:
				System.out.printf("Found at %dnd Transformation!! %n%n",shortestTrans);
				break;
			case 3:
				System.out.printf("Found at %drd Transformation!! %n%n",shortestTrans);
				break;
			default:
				System.out.printf("Found at %dth Transformation!! %n%n",shortestTrans);
				break;
			}			
			...
			System.out.printf("Not Found!!%n%n");
			return 0;
		}
	}
